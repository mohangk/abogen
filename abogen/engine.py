...
class ConversionEngine:
    ...
    def __init__(
        ...
        self.progress_callback = progress_callback or (lambda val, etr: None)
        self.finished_callback = finished_callback or (lambda res, path: None)
        ...
        self.replace_single_newlines = True
        self.use_silent_gaps = False
        self.subtitle_speed_method = "tts"
    ...
    def run(self):
        ...
        try:
            hf_tracker.set_log_callback(lambda msg: self.log_callback(msg))
            # Show configuration
            self.log_callback("Configuration:")
            ...
            self.log_callback(f"- Input File: {input_file}")
            if input_file != processing_file:
                self.log_callback(f"- Processing File: {processing_file}")
            ...
            if hasattr(self, "file_size_str"):
                self.log_callback(f"- File size: {self.file_size_str}")

            self.log_callback(f"- Total characters: {int(self.total_char_count):,}")

            self.log_callback(
                f"- Language: {self.lang_code} ({LANGUAGE_DESCRIPTIONS.get(self.lang_code, 'Unknown')})"
            )
            self.log_callback(f"- Voice: {self.voice}")
            self.log_callback(f"- Speed: {self.speed}")
            self.log_callback(f"- Subtitle mode: {self.subtitle_mode}")
            self.log_callback(f"- Output format: {self.output_format}")
            self.log_callback(
            ...
            self.log_callback(f"- Save option: {self.save_option}")
            if self.replace_single_newlines:
                self.log_callback(f"- Replace single newlines: Yes")
            ...
            if is_subtitle_input:
                if getattr(self, "use_silent_gaps", False):
                    self.log_callback("- Use silent gaps: Yes")
                ...
                self.log_callback(f"- Speed adjustment method: {method_label}")
            ...
            if not self.is_direct_text and self.file_name:
                ...
                if file_ext in [".srt", ".ass", ".vtt"]:
                    ...
                    self.log_callback(f"Chapters detected: -1")
                    # Wait for user response using event with timeout for responsive cancellation
                    # Auto-accept timestamps in CLI
                    is_timestamp_text = True
                    self.log_callback("Timestamps detected and enabled.")
            ...
            if (
                ...
                # Auto-accept defaults in CLI
                self.log_callback(f"Chapters detected: {total_chapters}. Using default options.")
                ...
                for text_segment in text_segments:
                    for result in tts(
                        ...
                        if self.cancel_requested:
                            ...
                            self.finished_callback("Cancelled", None)
                            return
                        current_segment += 1
                        ...
                        self.progress_callback(percent, etr_str)
            ...
            if merge_chapters_at_end:
                ...
                if self.output_format in ["wav", "mp3", "flac"]:
                    ...
                    ffmpeg_proc.stdin.close()
                    ffmpeg_proc.wait()
                self.progress_callback(100, "00:00:00")
            ...
            try:
                ...
                pass
            self.log_callback((f"Error occurred: {str(e)}", "red"))
            self.finished_callback(("Audio generation failed.", "red"), None)
    ...
    def _process_subtitle_file(self, tts, base_path, is_timestamp_text=False):
        ...
        try:
            ...
            if not subtitles:
                self.log_callback(("No valid subtitle entries found.", "red"))
                self.finished_callback(
            ...
            self.finished_callback((result_msg, "green"), merged_out_path)
            ...
            try:
                ...
                pass
            self.log_callback((f"Error processing subtitle file: {str(e)}", "red"))
            self.finished_callback(("Audio generation failed.", "red"), None)
